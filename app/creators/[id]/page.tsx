...
export default async function CreatorDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="container mx-auto px-6 py-12 max-w-7xl min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-8 bg-gray-900/60 p-8 rounded-2xl border border-gray-800 shadow-xl backdrop-blur-sm mb-12 relative overflow-hidden">
        ...
        <div className="w-full md:w-1/3 lg:w-1/4 shrink-0">
          <div className="aspect-square rounded-xl overflow-hidden border-2 border-pink-500/30 shadow-[0_0_20px_rgba(236,72,153,0.15)] bg-gray-950">
            <img
        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
            <div>
              ...
              {creator.gg_code && (
                <div className="mt-2 inline-block bg-gray-950 border border-gray-800 text-cyan-400 font-mono text-sm px-3 py-1 rounded-md">
                  Use Code: <span className="font-bold text-white">{creator.gg_code}</span>
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3">
              {socialLinks.twitch && (
                <a href={socialLinks.twitch} target="_blank" rel="noreferrer" className="bg-[#6441a5] hover:bg-[#7d5bbe] text-white px-4 py-2 rounded-lg font-bold transition-all shadow-lg text-sm">
                  Twitch
                </a>
              )}
              {socialLinks.youtube && (
                <a href={socialLinks.youtube} target="_blank" rel="noreferrer" className="bg-[#FF0000] hover:bg-[#ff3333] text-white px-4 py-2 rounded-lg font-bold transition-all shadow-lg text-sm">
                  YouTube
                </a>
              )}
              {socialLinks.twitter && (
                <a href={socialLinks.twitter} target="_blank" rel="noreferrer" className="bg-[#1DA1F2] hover:bg-[#40b1f5] text-white px-4 py-2 rounded-lg font-bold transition-all shadow-lg text-sm">
                  Twitter
                </a>
              )}
            </div>
