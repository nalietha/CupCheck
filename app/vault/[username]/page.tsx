...
export default function VaultPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  const getShelfItems = (shelf: ShelfChoice, allVaultItems: VaultItem[]) => {
    ...
    switch (shelf.type) {
      case 'preset':
        if (shelf.id === 'favorites') {
          items = allVaultItems.filter(item => item.is_favorite);
        } else if (shelf.id === 'newest') {
          items = [...allVaultItems].sort((a, b) => new Date(b.added_at).getTime() - new Date(a.added_at).getTime());
        }
        break;
      case 'collection':
    ...
    return items.slice(0, 5); // Limit shelf display to 5 items
  };
  ...
  return (
    <div className="min-h-screen bg-vaporBg pb-20">
      <VaultHeader
        ...
        /* Fallback logic prioritizes custom date */
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 space-y-12">
        ...
        <div className="space-y-8">
          {profile.vault_shelves?.map((shelf: ShelfChoice) => {
          ...
          {/* Fallback if they haven't set any shelves yet */}
          {(!profile.vault_shelves || profile.vault_shelves.length === 0) && (
            <VaultDisplayShelf
              title="Favorites"
